<template>
  <div>
    ...
    <div v-for="level1 in level1List" :key="level1.sysPermissionId">
      <a class="menu-title is-size-7" v-show="level1.type == 3" @click="onClickModule(level1.sysPermissionId)">
      ...
      {{activeModule}}
      <!-- v-show="activeLevel1 == level1.sysPermissionId"-->
      <div class="side-menu is-size-7 active" v-if="level1.type == 3">
        <!--如果是菜单，就不在显示子菜单-->
        <!--
        <router-link v-for="level2 in level1.children" v-if="!level2.hidden && level1.type == 1"
        ...
        <router-link v-for="level2 in level1.children" v-if="!level2.hidden"
                     :to="{path: level2.path}"
                     v-show="activeLevel1 == level1.sysPermissionId || activeModule  == level1.sysPermissionId "
